...
/****** Object:  Job [B_BILL_PAY_ATM_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 7/24/2025 8:34:00 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_BILL_PAY_ATM_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Bill Payment On-Us ATM', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'KASIKORNBANK\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
...
BEGIN
    ...
    (
        ...
        ''THB'', -- CCYCODE
        ''TH'', -- REGION
        ''{"fee_code01":"BIL_PAY_FEE","fee_code02":"BIL_PAY_CHG"}'', -- FEECODE
        25.00, -- FEEVAL01
        25.00, -- FEEVAL02
        ''ATM_BIL_PAY'', -- COMCODE
        ''Y'', -- ISONUS
        ...
        ''{
            "tx_desc":"Bill Payment",
            "com_code":"ATM_BIL_PAY",
            "p_code":"ATM_BIL_PAY",
            "ccycode":"THB"
        }'', -- REFVAL10
		...
		(
			SELECT 
				 GETUTCDATE() AS [billdate],
				 GETDATE() AS [paydate],
				 (SELECT TOP 1 CODEID+''-''+CODEVAL FROM D_CODETYPE WHERE CODESUBTYPE = ''BILL'') AS [billtype],
				 ''0000''+SUBSTRING(CAST(ABS(CHECKSUM(NEWID())) AS nvarchar(max)),1,8) AS [bill_no],
				 LOWER(NEWID()) AS [uuid]
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_BDY_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
...
SET
    ...
    INCACNO     = ''{"ac_info":{"sercode":"ATM_BIL_PAY","gl_acno":[{"ac_no":"11000070"}],"txcode":"0020","ccy_id":"THB"}}'',        -- Empty JSON as string
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_STS_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_BAL_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_API_STM_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_WFL_STM_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
...
DECLARE row_cursor CURSOR FOR
    SELECT RECID
    FROM D_CDSTM 
    WHERE CHAR20 IS NULL AND TXCODE = ''0020'';
...
BEGIN
    ...
    SELECT 
        (SELECT * 
         FROM D_CDSTM
         WHERE RECID = @ID AND CHAR20 IS NULL AND TXCODE = ''0020''
...
GO', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_COM_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:34:00 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_BILL_PAY_ATM_EXEC_AM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
		...
		@schedule_uid=N'f650877b-4914-48b1-b067-ac2af40ab270'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_BILL_PAY_ATM_EXEC_PM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'5bc90e75-1167-40f4-8da1-2579d642d270'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
