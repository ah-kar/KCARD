...
/****** Object:  Job [B_SYC_CRD_CAD_EXEC]    Script Date: 7/24/2025 8:37:38 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 7/24/2025 8:37:38 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_SYC_CRD_CAD_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Sync KCard Credit Card Infor Job', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'KASIKORNBANK\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_CARD_INFO_EXEC]    Script Date: 7/24/2025 8:37:38 PM ******/
...
BEGIN
	...
	''C'',
	(SELECT TOP 1 CODEID FROM D_CODETYPE WHERE CODETYPE = ''CREDITCARD'' AND CODESUBTYPE = ''CCTYPE'' ORDER BY NEWID()),
	''P'',
	@RandomString,
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_CARD_INFO_EXEC]    Script Date: 7/24/2025 8:37:38 PM ******/
...
UPDATE D_CARDLST
SET 
CARDNO = CONCAT(''93'',RIGHT(''00000000'' + CAST(RECID AS NVARCHAR), 8)),
...
WHERE CARDSTS = ''P'' AND ISVERIFY = ''P'' AND CARDTYPE = ''C''
COMMIT TRAN;', 
		@database_name=N'master', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ECY_CARD_PWD_EXEC]    Script Date: 7/24/2025 8:37:38 PM ******/
...
WHERE HASHPIN IS NULL AND CARDTYPE = ''C'';
GO
-- close key
...
GO', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_SYS_LOG_EXEC]    Script Date: 7/24/2025 8:37:38 PM ******/
...
SELECT RECID
FROM D_CARDLST WHERE CARDSTS = ''P'' AND ISVERIFY = ''P'' AND CARDTYPE = ''C''
...
BEGIN
    ...
    SELECT 
        (SELECT *
         FROM D_CARDLST
         WHERE RECID = @ID AND CARDSTS = ''P'' AND ISVERIFY = ''P'' AND CARDTYPE = ''C''
...
ACTION_TYPE = ''Sync Credit Card/Insert'',
...
commit tran;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_CARD_LOG_EXEC]    Script Date: 7/24/2025 8:37:38 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_SYC_CRD_CAD_EXEC', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=62, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'dcffb8cf-7807-4d43-9b5c-324db35f5739'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
