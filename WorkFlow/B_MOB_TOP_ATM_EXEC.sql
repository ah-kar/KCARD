...
/****** Object:  Job [B_MOB_TOP_ATM_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 7/24/2025 8:37:17 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_MOB_TOP_ATM_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Mobile TopUp/Reload On-US ATM', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'KASIKORNBANK\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
...
BEGIN
    ...
    (
        ...
        (SELECT TOP 1 CODEID
         FROM (
             SELECT CODEID FROM D_CODETYPE WHERE CODETYPE = ''TXAMT'' AND CODESUBTYPE = ''TOPUP''
             UNION ALL
             SELECT CODEID FROM D_CODETYPE WHERE CODETYPE = ''TXAMT'' AND CODESUBTYPE = ''TOPUP'' AND CODEID = ''50''
             UNION ALL
             SELECT CODEID FROM D_CODETYPE WHERE CODETYPE = ''TXAMT'' AND CODESUBTYPE = ''TOPUP'' AND CODEID = ''100''
             UNION ALL
             SELECT CODEID FROM D_CODETYPE WHERE CODETYPE = ''TXAMT'' AND CODESUBTYPE = ''TOPUP'' AND CODEID = ''20''
         ) AS WeightedPool
         ORDER BY NEWID()), -- AMT
        ...
        ''{
            "tx_desc":"Mobile Top-up / Reload",
            "com_code":"ATM_TOP_MOB",
            "p_code":"ATM_TOP_MOB",
            "ccycode":"THB"
        }'', -- REFVAL10

        ''Mobile Top-up / Reload'', -- CHAR02
        0.00, -- NUM03
        0.00, -- NUM04
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_BDY_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_STS_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_BAL_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_API_STM_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_WFL_STM_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
...
DECLARE row_cursor CURSOR FOR
    SELECT RECID
    FROM D_CDSTM 
    WHERE CHAR20 IS NULL AND TXCODE = ''0021'';
...
BEGIN
    ...
    SELECT 
        (SELECT * 
         FROM D_CDSTM
         WHERE RECID = @ID AND CHAR20 IS NULL AND TXCODE = ''0021''
...
GO', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_COM_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:37:17 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_MOB_TOP_ATM_EXEC_AM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'33430e18-6b89-4c01-89b1-1a7e3c7af405'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_MOB_TOP_ATM_EXEC_PM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'fcff6194-4532-4a4c-9885-8883eca6da5f'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
