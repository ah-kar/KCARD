...
/****** Object:  Job [B_WOF_ATM_TXN_EXEC]    Script Date: 7/24/2025 8:37:57 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 7/24/2025 8:37:57 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_WOF_ATM_TXN_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Cash Withdrawal – Off-Us ATM', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'KASIKORNBANK\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:37:58 PM ******/
...
BEGIN
    ...
    (
        ...
        ''0002'', -- TXCODE
        ''DPT_ATM_WTD_OFFUS'', -- TRANCODE
        ''901'', -- BCODE
        ''CARD_PAY'', -- PAYCODE
        ''-'', -- PAYREFNO
        ''XXXX-XXXX'', -- BANKID
        ''XXX-XXX-XXX'', -- SWIFTCODE
        ''-'', -- MERCHANTID
        ...
        ''THB'', -- CCYCODE
        ''TH'', -- REGION
        ''{"fee_code01":"WTD_OFFUS_FEE","fee_code02":"WTD_OFFUS_CHG"}'', -- FEECODE
        15.00, -- FEEVAL01
        15.00, -- FEEVAL02
        ''ATM_WTD_OFFUS'', -- COMCODE
        ''N'', -- ISONUS
        ''Y'', -- ISOFFUS
        ''CSH_WTD_OFFUS'', -- PCODE
        ''Online'', -- TRANTYPE
        ...
        ''WF_CAD_PAY'', -- WFCODE
        ''Cash Withdrawal – Off-Us ATM'', -- DESCS
        ...
        (SELECT TOP 1 TMNID, TMNCODE, TMNTYPE,BANKID
         FROM D_TMNLST 
         WHERE TMNTYPE = ''A'' AND ISOFFUS = ''Y'' 
         ORDER BY NEWID() 
         FOR JSON PATH), -- REFVAL02

        ''{
            "tx_desc":"Cash Withdrawal – Off-Us ATM",
            "com_code":"ATM_WTD_OFFUS",
            "p_code":"CSH_WTD_OFFUS",
            "ccycode":"THB"
        }'', -- REFVAL10

        ''Cash Withdrawal – Off-Us ATM'', -- CHAR02
        0.00, -- NUM03
        0.00, -- NUM04
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_BDY_EXEC]    Script Date: 7/24/2025 8:37:58 PM ******/
...
SET
    ...
    INCACNO     = ''{"ac_info":{"sercode":"ATM_WTD_OFFUS","gl_acno":[{"ac_no":"11009001"},{"ac_no":"41008001"}],"txcode":2,"ccy_id":"THB"}}  '',        -- Empty JSON as string
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_STS_EXEC]    Script Date: 7/24/2025 8:37:58 PM ******/
...
SET 
    ...
    WFCODE = CASE 
             ...
             END,
	SWIFTCODE = CASE 
				WHEN BANKID = ''SCB'' THEN ''SICOTHBK''
				WHEN BANKID = ''BBL'' THEN ''BKKBTHBK''
				WHEN BANKID = ''KTB'' THEN ''KRTHTHBK''
				WHEN BANKID = ''BAY'' THEN ''AYUDTHBK''
				WHEN BANKID = ''GSB'' THEN ''GOVBTHBK''
				WHEN BANKID = ''TMB'' THEN ''TMBKTHBK''
				WHEN BANKID = ''UOBT'' THEN ''UOVBTHBK''
	END
WHERE 
    TXCODE = ''0002''
    AND STS IS NULL;
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_BAL_EXEC]    Script Date: 7/24/2025 8:37:58 PM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_API_STM_EXEC]    Script Date: 7/24/2025 8:37:58 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_WFW_STM_EXEC]    Script Date: 7/24/2025 8:37:58 PM ******/
...
DECLARE row_cursor CURSOR FOR
    SELECT RECID
    FROM D_CDSTM 
    WHERE CHAR20 IS NULL AND TXCODE = ''0002'';
...
BEGIN
    ...
    SELECT 
        (SELECT * 
         FROM D_CDSTM
         WHERE RECID = @ID AND CHAR20 IS NULL AND TXCODE = ''0002''
...
GO', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_COM_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:37:58 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_WOF_ATM_TXN_EXEC_AM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
		...
		@schedule_uid=N'04702b68-1bfa-45c5-98af-46a0aa893077'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_WOF_ATM_TXN_EXEC_PM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'6d5fc716-2e1d-4154-a29a-194ed89e3ed8'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
