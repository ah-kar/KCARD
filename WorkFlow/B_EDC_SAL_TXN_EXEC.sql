...
/****** Object:  Job [B_EDC_SAL_TXN_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 7/24/2025 8:35:46 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_EDC_SAL_TXN_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'EDC Purchase', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'KASIKORNBANK\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
...
BEGIN
    ...
    (
        ...
        ''THB'', -- CCYCODE
        ''TH'', -- REGION
        ''{"fee_code01":"EDC_FEE","fee_code02":"EDC_CHG"}'', -- FEECODE
        5.00, -- FEEVAL01
        5.00, -- FEEVAL02
        ''EDC_SAL'', -- COMCODE
        ''N'', -- ISONUS
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_BDY_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
...
SET
    ...
    INCACNO     = ''{"ac_info":{"sercode":"EDC_PAY","gl_acno":[{"ac_no":"21007002"}],"txcode":"0011","ccy_id":"THB"}}'',        -- Empty JSON as string
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_STS_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_BAL_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_API_STM_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_WFW_STM_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
...
DECLARE row_cursor CURSOR FOR
    SELECT RECID
    FROM D_CDSTM 
    WHERE CHAR20 IS NULL AND TXCODE = ''0011'';
...
BEGIN
    ...
    SELECT 
        (SELECT * 
         FROM D_CDSTM
         WHERE RECID = @ID AND CHAR20 IS NULL AND TXCODE = ''0011''
...
GO', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_COM_TXN_INFO_EXEC]    Script Date: 7/24/2025 8:35:46 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_EDC_SAL_TXN_EXEC_AM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
		...
		@schedule_uid=N'285fd7f3-e23b-4e34-8974-d61253df1872'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_EDC_SAL_TXN_EXEC_PM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'2858dfc1-525d-45fb-b113-c874e64e6b96'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
