...
/****** Object:  Job [B_WOF_CLS_TXN_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 7/25/2025 4:57:47 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_WOF_CLS_TXN_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Cardless Withdrawal (QR Scan) – Off-Us ATM', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'KASIKORNBANK\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_INFO_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
...
BEGIN
    -- Declare and generate the 255-character random string
    ...
    (
        ...
        [CHAR02], [NUM03], [NUM04], [DATE01], [DATE2], [DATE03], [REFVAL05], [CHAR04]
    )
    VALUES
    (
        LOWER(NEWID()),
        ...
        ''kcard'',
        ''8073c0a9-3cc8-40a9-b3e8-214a1876450b'',
        ...
        ''KASITHBK'',
        ''-'',
        ''noreply-kcard@kbank.com,info-kcard@kbank.com'',
        (SELECT TOP 1 CODEID
         FROM (
        ...
        (SELECT TOP 1 CODEID
         ...
         ORDER BY NEWID()),
        ''WF_CAD_PAY'',
        ''Cardless Withdrawal (QR Scan) – Off-Us ATM'',
        ...
        (SELECT TOP 1 TMNID, TMNCODE, TMNTYPE 
         ...
         FOR JSON PATH),
        ''{
            "tx_desc":"Cardless Withdrawal (QR Scan) – Off-Us ATM",
            "com_code":"CLS_WTD_OFFUS",
            "p_code":"QRS_WTD_OFFUS",
            "ccycode":"THB"
        }'',
        ''Cardless Withdrawal (QR Scan) – Off-Us ATM'',
        0.00,
        ...
        GETUTCDATE(),
        ''{"tx_channel":"ATM"}'',
        (SELECT @RandomString AS qr_code,''A'' sts for json path,WITHOUT_ARRAY_WRAPPER) -- CHAR04: the generated 255-char random string
    );
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TXN_BDY_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_STS_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TXN_BAL_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_API_STM_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_WFL_STM_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
...
DECLARE row_cursor CURSOR FOR
    SELECT RECID
    FROM D_CDSTM 
    WHERE CHAR20 IS NULL AND TXCODE = ''0024'';
...
BEGIN
    ...
    SELECT 
        (SELECT * 
         FROM D_CDSTM
         WHERE RECID = @ID AND CHAR20 IS NULL AND TXCODE = ''0024''
...
GO', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_COM_TXN_INFO_EXEC]    Script Date: 7/25/2025 4:57:47 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_WOF_CLS_TXN_EXEC_AM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'13d37c44-3b60-40e3-a158-55198f20bc8b'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_WOF_CLS_TXN_EXEC_PM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
		...
		@schedule_uid=N'dc3eee12-7d69-4572-8807-9c9291ccec7c'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
